...
PROJECT_NAME="ai-quickstart-anythingllm"
...
notify "⬇️ Downloading latest vLLM & AnythingLLM container images... (this may take 2 - 3 min)..."
cd /opt/${PROJECT_NAME}
...
if [ -f "/lib/modules/${CURRENT_KERNEL}/kernel/nvidia-580-open/nvidia.ko" ] || \
   [ -f "/lib/modules/${CURRENT_KERNEL}/updates/dkms/nvidia.ko" ]; then
    ...
    if nvidia-smi > /dev/null 2>&1; then
        ...
        cd /opt/${PROJECT_NAME}
        notify "🚀 Starting vLLM & AnythingLLM with docker compose up ..."
        docker compose up -d
        exit 0
    fi
fi
